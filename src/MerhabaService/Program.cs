...
public static class Program
{
    public static void Main()
    {
        ...
        app.MapGet("/postStart", (ILogger<MerhabaApp> logger) 
            => logger.LogInformation("PostStart event"));
        app.MapGet("/preStop", (ILogger<MerhabaApp> logger) 
            => logger.LogInformation("PreStop event"));

        app.MapGet("/", (IConfiguration configuration) => 
        {
            var message = configuration["MERHABA_MESSAGE"] 
                ?? "Merhaba! env var not found.";
            return new Merhaba(message);
        });
...
public record Merhaba(string Message);
